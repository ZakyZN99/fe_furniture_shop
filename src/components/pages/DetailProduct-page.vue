    ...
    <div>
        ...
        <div class="h-[100px] bg-[#642C0C] px-[100px] py-[39px]">
            <div class="flex items-center text-[#ffff]">
                    ...
                    <h2 class="mx-[16px] text-[#FFFFFF]  open-sans text-[16px]">{{product?.name}}</h2>
            </div> 
        </div>
<!-- DETAIL PRODUCT -->
        <div class="pl-[100px] pb-[100px] ">
            <div class="bg-[#ffffff] flex gap-[106px]">
                <div class=" pt-[52px] w-[100%]  ">
                    <img :src="product?.image" alt="Product Image" class="w-[605px] h-[447px] "/>
                ...
                <div class= "pt-[52px] w-[100%] ">
                    <h1 class="text-[42px] text-[#642C0C]">{{product?.name}}</h1>
                    <p class="text-[#642C0C] open-sans font-semibold text-[24px]">Rp. {{ product?.price.toLocaleString() }}</p>
                    <div class="flex  pt-[20px] gap-[20px] items-center">
                        ...
                        <p class="text-[#E5D097] open-sans text-[13px]">{{ product?.reviews }} Customers Review</p>
                    </div>
                    <p class="text-[#CCC4B4] text-[16px] open-sans pt-[12px] pr-[237px]">{{product?.description}}</p>
                    <h2 class="text-[#642C0C] open-sans text-[14px] pt-[28px]">Size</h2>
                    <div class="flex gap-[16px] pt-[14px] pb-[40px]">
                        <button @click="setSize('XS')" :class="{ 'bg-[#642C0C] text-[#FFFFFF]': product?.size === 'XS', 'bg-[#CCC4B4] text-[#642C0C]': product?.size !== 'XS' }" class="w-[30px] h-[30px] poppins rounded-md">XS</button>
                        <button @click="setSize('L')" :class="{ 'bg-[#642C0C] text-[#FFFFFF]': product?.size === 'L', 'bg-[#CCC4B4] text-[#642C0C]': product?.size !== 'L' }" class="w-[30px] h-[30px] poppins rounded-md">L</button>
                        <button @click="setSize('XL')" :class="{ 'bg-[#642C0C] text-[#FFFFFF]': product?.size === 'XL', 'bg-[#CCC4B4] text-[#642C0C]': product?.size !== 'XL' }" class="w-[30px] h-[30px] poppins rounded-md">XL</button>
                    </div>
                    <div class="border-[2px] w-[119px] h-[51px] border-[#642C0C] flex items-center">
                        ...
                        <span class="text-[#642C0C] text-[24px] open-sans  px-[24px]">{{ product?.quantity }}</span>
                    ...
                    <div class="pt-[38px] flex items-center pb-[42px]">
                        <div class="flex items-center">
                            <button class="w-[171px] items-center flex gap-[8px] h-[51px] bg-[#C0772C]"  @click="addToCart(product)">
                        ...
                        <button class="pl-[26px]" @click="toggleFavorite">
                            <component :is="product?.isFavorite ? 'AnFilledHeart' : 'AnOutlinedHeart'" class="size-[40px] text-[#C0772C]" />
                        </button>
                    </div>
    ...
    mounted() {
        ...
        fetch(`/dataJson/dummyProducts.json`)
            ...
            .then(data => {
                ...
                console.log(product)
                if (product) {
                    product.image = require(`@/assets/img/${product.image}`);
